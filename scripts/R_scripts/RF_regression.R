...
################################
##MTurk 2 Principal Components##
################################
df_labels <- read.csv("datasets/all-places-mturk-2_pc-labels.csv")
labels <- colnames(df_labels[,!(colnames(df_labels) %in% c("annotation"))])
...
saveRDS(df.fit.summary, "r_results/regression_ss_2_pcs_results.rds")
